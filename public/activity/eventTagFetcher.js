var EventTypeFilter = async(id)=>{
  try{
    const response = await fetch(`https://gdg.community.dev/api/chapter/748/event?order_by=start_date&start_date=${todayDate()}&end_date=9999-12-31&fields=title,audience_type,description_short,start_date,end_date,picture,tags,tickets,url,event_type_title,audience_type,event_type&event_type_id=${id}`);
...
let eventReader = async (ActivityList) => {
